...
If you're on Ubuntu with upstart (maybe 12.x?? through 14.x) and you want to
inflict this on all new sessions, do this.
...
Log out of your session, then log back in.  C<fyfsp> should be running now, I
messed up the installer, you're not on Ubuntu, or something else went wrong.

=head2 build and install just for you

If you just want it for you, probably the most rational choice on a multi-user
system — people still have this? — then you'd do this instead.
...
=head2 build, install, and start it
 ...
 make install
 copy upstart.conf ~/.config/upstart
 start fyfsp
